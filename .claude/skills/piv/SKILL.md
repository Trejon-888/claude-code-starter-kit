...
# /piv - Self-Recursive Development Loop v3.4

**The ultimate command for automated, validated, production-ready development.**

## Composable Commands

PIV integrates with standalone commands. Use them independently or as part of the full loop:

| Command | Purpose | When to Use Standalone |
|---------|---------|----------------------|
| `/plan` | Deep codebase analysis → implementation plan | Planning without full PIV |
| `/execute` | Autonomous plan execution | Implementing a known plan |
| `/review` | Code review on git diff | Pre-commit quality check |
| `/system-review` | Plan vs reality analysis | Post-implementation process improvement |
| `/validate` | 5-level validation | Quick health check |
| `/prime` | Load project context | New session orientation |
| `/worktree` | Parallel development | Multiple independent features |
| `/fix-issue` | GitHub issue → branch → fix → PR | Bug fix workflow |

**Composition examples:**
- Quick fix: `/fix-issue 42` (end-to-end)
...
When creating plans (Phase 1), use these keywords for clarity:
...
Per task: **IMPLEMENT** / **PATTERN** / **IMPORTS** / **GOTCHA** / **VALIDATE**
